...
export async function GET(): Promise<NextResponse> {
  ...
  try {
    ...
    await prisma.pricingSetting.upsert({
      where: { id: "ps_default" },
      create: {
        id: "ps_default", label: "默认配置",
        seaPrice: 580, landPrice: 620,
        seaMinCharge: 100, landMinCharge: 80, airMinCharge: 200,
        minSeaCBM: 0.5, minLandCBM: 0.3, minAirKG: 5,
        oceanFreightPerContainer20: 3500, oceanFreightPerContainer40: 6800,
        airPrice: 15
      },
      update: {}
    });

    // Seed TransportBills (4 XT-prefix formal bills)
    const bills = [
    ...
    return NextResponse.json({
      ok: true, message: "Database seeded successfully",
      users, bills: bills2, pricing
    });
  } catch (error: any) {
