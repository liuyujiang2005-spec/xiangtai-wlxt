...
import { Download, Search, ShieldCheck, PackageCheck, ChevronLeft, ChevronRight } from "lucide-react";
...
export default function TransportBillsPage() {
  ...
  /**
   * 基于搜索词、预报筛选与分页参数拉取运单列表。
   */
  const fetchBills = useCallback(
    async (searchTerm: string, onlyForecast: boolean, pageIndex: number): Promise<void> => {
      setLoading(true);
      setErrorMessage("");
      try {
        ...
        const data = (await response.json()) as { list?: BillRow[]; pagination?: Pagination };
  ...
  useEffect(() => {
    void fetchBills(appliedQuery, forecastOnly, pagination.page);
  }, [forecastOnly, pagination.page, appliedQuery, fetchBills]);
  ...
  async function handleSearchSubmit(
    ...
    if (newQuery !== appliedQuery) {
      ...
      if (pagination.page !== 1) {
        ...
        await fetchBills(newQuery, forecastOnly, 1);
      }
    }
  }
      ...
      <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center">
        ...
        >
          ...
          <button
            type="submit"
            disabled={loading}
            className="rounded-xl bg-brand px-3 py-1.5 text-sm font-medium text-white disabled:opacity-50"
          >
            {loading ? "搜索中..." : "搜索"}
          </button>
        </form>
