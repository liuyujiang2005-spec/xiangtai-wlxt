...
async function main(): Promise<void> {
  ensureLocalOpsAllowed("password:reset");
  const username = (process.argv[2] ?? "").trim();
  const plain = process.argv[3] ?? "";
  if (!username || !plain) {
    console.error(
      "用法: ACCOUNT_OPS_CONFIRM=YES npx tsx scripts/reset-password.ts <登录名> <新密码>\n示例: ACCOUNT_OPS_CONFIRM=YES npx tsx scripts/reset-password.ts Allen <新密码>"
    );
    process.exitCode = 1;
    return;
  }
