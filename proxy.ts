...
/**
 * 已登录用户访问无权页面时，回到自己的首页并保留提示参数。
 */
function redirectToRoleHomeWithForbidden(
  request: NextRequest,
  role: "ADMIN" | "STAFF" | "CLIENT"
): NextResponse {
  const target = new URL(getHomePathForRole(role), request.url);
...
export async function proxy(request: NextRequest): Promise<NextResponse> {
  ...
  if (pathname === "/login") {
    if (session) {
      const target = new URL(getHomePathForRole(session.role), request.url);
      if (request.nextUrl.searchParams.get("error") === "forbidden") {
        target.searchParams.set("error", "forbidden");
      }
      return NextResponse.redirect(target);
    }
  ...
  if (
    ...
    if (pathname.startsWith("/admin") && session.role !== "ADMIN") {
      return redirectToRoleHomeWithForbidden(request, session.role);
    }
